...
* Load the `md_gov_county.csv` and `md_county_gov_18.csv` files in the data folder
...
**A1.** 
https://datawrapper.dwcdn.net/p38Tv/1/


```{r}
md_gov_county <- read_csv("data/md_gov_county.csv")
md_county_gov_18 <- read_csv("data/md_gov_county_18.csv")
```
...
joined_md_counties <- md_gov_county %>% 
  left_join(md_county_gov_18, by = c("GEOID", "County")) %>% 
  mutate(total_Cox = (Cox + Moore + Lashar + Wallace + Harding + `Write-ins.x`)) %>% 
  mutate(pct_cox = Cox/total_Cox*100) %>% 
  mutate(total_Hogan = (Hogan + Jealous + Quinn + Schlakman + `Write-ins.y`)) %>% 
  mutate(pct_Hogan = Hogan/total_Hogan*100)

```

```{r}
#Exporting Data
write_csv(joined_md_counties, "data/joined_md_counties.csv")
```
...
**A2.** For me the percentage by mail in 2020 looks low. I believe due to Covid19, most people would prefer to either vote by mail or early, but interestingly the percentage for both are low. Also why don't we have the mail in ballot here?

https://datawrapper.dwcdn.net/Db9bf/1/
...
**A3.** 
Since I don't know about most area's zip codes, I am curious about the areas where the color is silver. Does this mean that there's no donor from those areas or those areas don't have any democrats? Also it would be interesting to have the data about who are these donors 

https://datawrapper.dwcdn.net/RcTSq/1/
...
md_actblue_zip <- md_actblue_zip %>% 
  mutate(grand_total = total + zip) %>% 
  mutate(pct_total = zip/grand_total*100)
