...
**Answer** It says one or more parsing issues and I believe it points out to  Rows 476915 and columns 13
...
**Answer** The number of rows presented is 476915 and the problems is with the character "true, false" and the number "1 or 2" but the actual number is 20, 10, etc. 
...
**Answer** No not error but there are issues with the column types or set
...
**Answer** Precinct is "chr", district is "dbl" and mail is "dbl"
```{r}
...
**Answer**: 402345, which means that this number of rows in dataframe has no value in mail column. 
```{r}
...
**Answer** the year is odd. Those in charge of entering data might have uploaded the wrong year/number. 
```{r}
yadkin_voters %>% 
...
**Answer**
Yes especially the ones whose status are duplicated
```{r}
...
**Answer** column names, city name and an address is spelled differently.
...
**Answer** it changed to x1_linenumber which might be readable and looking organized to the system?
...
**Answer** There's a problem while computing total_amount = sum (amount). It also says there's an invalid type (chr) of argument
...
**Answer** two and the name is Derrick Hamilton
...
**** The distinct function removes duplicated rows [exact similar rows] and column by showing the unique names, dates, etc. 
...
**** 1L - 5L -> the character L stands for left

**Answer** the number changed to five
...
**** str_detect means Ctrl+F which is to find words/characters
user "^" before the name for pattern match and not exact match.
