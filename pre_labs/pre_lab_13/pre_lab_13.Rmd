...
**Answer** 
The headers are long and there are comas between numbers and the totals include both numbers and percentage of the numbers
...
**Answer** 
Its cleaned now: short headlines, no commas in numbers and the total row is gone.
...
**Task** Right-click on the first red dot next to "Agriculture" and choose "Inspect". You should see multiple <li> tags inside a <ul> (ul stands for unorganized element) tag.
...
### Task 11: Isolate the text of `li` tags inside the HTML using `html_text`
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output:

**Answer** 
Now all 57 are visible but there are \n before each word in each row
...
### Task 12: Turn the contents of the <li> tags into a dataframe
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output:
**Answer** 
In some of the columns, there's space before each word, but in some others there's not space and I believe that's because of \n
```{r}
...
### Task 13: Remove the rows we don't need from our dataframe
